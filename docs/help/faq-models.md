...
<AccordionGroup>
  ...
  <Accordion title="Can I use GPT 5.5 for daily tasks and Codex 5.5 for coding?">
    ...
    - **Native Codex coding agent:** set `agents.defaults.model.primary` to `openai/gpt-5.6-sol`. Sign in with `openclaw models auth login --provider openai` when you want ChatGPT/Codex subscription auth.
    ...
    - **OpenAI agent API-key auth:** use `/model openai/gpt-5.6-sol` with an ordered `openai` API-key profile.
    - **Sub-agents:** route coding tasks to a Codex-focused agent with its own `openai/gpt-5.6-sol` model.
