...
describe("extractQaFailureReplyText", () => {
  ...
  it("classifies explicit provider auth guidance as a failure", () => {
    expect(
      extractQaFailureReplyText(
        '⚠️ No API key found for provider "openai". You are authenticated with OpenAI Codex OAuth. Use openai/gpt-5.6-sol with the Codex OAuth profile, or set OPENAI_API_KEY for direct OpenAI API access.',
      ),
  ...
  it("classifies curated missing-key guidance as a failure", () => {
    expect(
      extractQaFailureReplyText(
        "⚠️ Missing API key for OpenAI on the gateway. Use `openai/gpt-5.6-sol` with the Codex OAuth profile, or set `OPENAI_API_KEY`, then try again.",
      ),
