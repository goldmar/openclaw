...
describe("qa suite transport helpers", () => {
  ...
  it("fails success-only waitForOutboundMessage calls when a classified failure reply arrives first", async () => {
    ...
    state.addOutboundMessage({
      to: "dm:qa-operator",
      text: '⚠️ No API key found for provider "openai". You are authenticated with OpenAI Codex OAuth. Use openai/gpt-5.6-sol with the Codex OAuth profile, or set OPENAI_API_KEY for direct OpenAI API access.',
      senderId: "openclaw",
      senderName: "OpenClaw QA",
    });
