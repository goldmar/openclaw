...
describe("sendMessageTelegram", () => {
  ...
  it("strips HTML when durable rich sends reject an invalid entity", async () => {
    botRawApi.sendRichMessage.mockRejectedValueOnce(createRichEntityInvalidError("EMAIL"));
    botApi.sendMessage.mockResolvedValueOnce({ message_id: 46, chat: { id: "123" } });

    await sendMessageTelegram("123", "Status: <b>blocked</b>", {
      cfg: { channels: { telegram: { richMessages: true } } },
      token: "tok",
      textMode: "html",
    });

    expect(botApi.sendMessage).toHaveBeenCalledWith("123", "Status: blocked");
  });
